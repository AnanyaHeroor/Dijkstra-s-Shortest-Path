...
public class CS400Graph<T> implements GraphADT<T> {
    ...
    protected class Path implements Comparable<Path> {
        ...
        public Path(Path source) {
          ...
          //loop through existing data sequence and add each item into the new LinkedList
    ...
    protected Path dijkstrasShortestPath(T start, T end) {        
        if (!containsVertex(start) || !containsVertex(end)) //If there is no start or end vertices
          throw new NoSuchElementException("There are no vertices"); //we can't find the shortest path if there are no vertices
        Vertex startNode = vertices.get(start); 
        PriorityQueue<Path> unsettled = new PriorityQueue<Path>(); //create a new priority queue for unvisited vertices
        HashSet <Vertex> settled = new HashSet<Vertex>(); //create a new hash set for the visited vertices
        Path returnPath = new Path(startNode); //create a new path from the start vertex
        unsettled.add(returnPath); //add start node's path to the unvisited priority queue
        while(!unsettled.isEmpty()) { //while there are unvisited vertices
          Path p = unsettled.remove(); //remove the unvisited vertices from the priority queue and create a new path
          settled.add(p.end); //add the end vertex to the settled hash set since it is visited
          Iterator<Edge> adjs = p.end.edgesLeaving.iterator(); //look at edges leaving the most recent end vertex
          while(adjs.hasNext()) { //while there are things next to the visited node
            Edge next = adjs.next(); //go to the next node
            if(!settled.contains(next.target)) { //if settled doesn't have anything next to it
              Path p2 = new Path(p); //create a new path
              p2.extend(next); //extend the new path to go onto the next vertex/edge
              unsettled.add(p2); //add the new path to the unsettled priority queue and then this will loop through each unvisited vertex
            }
          }
          if(p.end.data == end) { //if there are no more vertices
            //We're done
            returnPath = p; //this is the shortest path if there are no more vertices
            unsettled.clear(); //clear the unsettled priority queue since we have visited everything
          }
        }
        
        
        return returnPath; //return the shortest path
